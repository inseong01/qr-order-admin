...
import { useQueryAllOrderList, useQueryOrderMenuList } from '@/hooks/use-query/query';
...
export function useOrderTab() {
  ...
  const processedOrderItems = orderItemsQuery.data?.filter((o) => o.order.is_done) ?? [];
  const notProcessedOrderItems = orderItemsQuery.data?.filter((o) => !o.order.is_done) ?? [];

  const orders = isDone ? processedOrders : notProcessedOrders;
  const orderItems = isDone ? processedOrderItems : notProcessedOrderItems;
