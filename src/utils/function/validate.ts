...
function createMenuValue(data: NewMenu) {
  const newMenuSchema = z.object({
    ...
    name: z.string({ message: '메뉴 이름을 입력해주세요.' }),
...
function updateMenuValue(data: UpdateMenu) {
  const updatedMenuSchema = z.object({
    ...
    category_id: z.string({ message: '메뉴 분류를 선택헤주세요.' }).optional(),
    name: z.string({ message: '메뉴 이름을 입력해주세요.' }).optional(),
    price: z
      .number({ message: '가격을 입력해주세요.' })
      .nonnegative({ message: '가격은 양수이어야 합니다.' })
      .optional(),
    tag: z.string({ message: '태그를 선택해주세요.' }).optional(),
    img_url: z.string({ message: '올바르지 않은 이미지 주소입니다.' }).optional(),
  });
