...
import { childMotion, parentsMotion } from '@/lib/motion/widget';

import styles from '@/feature/widget/components/index.module.css';

export const widgetOpenAtom = atom(false);
...
export function WidgetIconButton() {
  ...
  function handleWidgetStatus() {
    ...
    setWidgetOpenStatus((prev) => !prev);
  }

  return (
    <ListBox key='widgetButton' onClick={handleWidgetStatus} isRow={true} isAnimate={false}>
      <span>{isWidgetOpen ? '닫기' : '열기'}</span>

      <div className={styles.iconBox}>
        <img src={isWidgetOpen ? '' : ''} alt='icon' />
      </div>
    </ListBox>
  );
}
