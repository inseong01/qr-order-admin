...
import {
  ...
  isSupabaseCalled,
} from './fixture/update-password.fixture';
 ...
 * @description [성공/실패] 비밀번호 재설정 기능의 End-to-End 시나리오를 검증합니다.
 ...
 */
test.describe('[성공/실패] 비밀번호 재설정', () => {
  test.describe('[성공] 비밀번호 재설정', () => {
    /**
     * [성공] 유효한 링크 접속 시 비밀번호 재설정 성공 및 로그인 페이지로 리다이렉트
     * - 인증 토큰 확인, 입력, 버튼 클릭, 토큰 제거, URL 이동, Supabase 호출 확인
     */
    updatePasswordSuccessTest(
  ...
  test.describe('[실패] 비밀번호 재설정', () => {
     ...
     */
    updatePasswordFailInvalidCaptchaTest(
      ' 캡챠 인증 실패 시, 입력 필드와 버튼은 비활성화되어야 한다',
      async ({ page }) => {
    ...
    /**
     * [실패] 잘못된 비밀번호 형식 입력 시 오류 메시지 표시
     * - 잘못된 입력, 오류 메시지, 버튼 상태 확인
     */
    updatePasswordFailInvalidFormatTest(
      '잘못된 비밀번호 형식 입력 시, 오류 메시지가 표시되어야 한다',
      async ({ page }) => {
        // 1. 비밀번호 재설정 페이지로 이동합니다.
        await page.goto(TEST_PAGE_URL);
    ...
    /**
     * [실패] 유효하지 않은 토큰으로 접속 시 토스트 알림 표시
     * - 잘못된 토큰, 토스트 알림, URL 이동 없음, Supabase 호출 확인
     */
    updatePasswordFailInvalidToken(
      '유효하지 않은 토큰으로 접속 시, 오류 메시지가 표시되어야 한다',
      async ({ page }) => {
        // 1. 비밀번호 재설정 페이지로 이동합니다.
        await page.goto(TEST_PAGE_URL);
