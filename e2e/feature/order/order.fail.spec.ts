...
import { orderTabAPITest_F1 } from './fixtures/index.fail.fixture';

/**
 * @file 주문 관리 E2E 테스트
 * @description [실패] 주문 기능의 End-to-End 시나리오를 검증합니다.
 *   - 신규 주문 접수, 완료 처리, 카드 UI 미반영, 토스트 알림 확인
 */
test.describe('[실패] 주문 관리', () => {
  /**
  ...
  /**
   * [실패] 신규 주문 접수 후 완료 처리 - 카드 사라지지 않고 완료 수 변동 없음
   * - 사용자 조작, API 모킹, 토스트 알림 확인, UI 미반영 검증
   */
  orderTabAPITest_F1('신규 주문 완료 처리 실패 - 주문 카드가 사라지지 않고 완료 수 변동 없음', async ({ page }) => {
    // 1. '주문' 탭 클릭
    ...
    await expect(page.getByText(`접수 ${mockInitOrders.filter((d) => !d.is_done).length}`)).toBeVisible();
    await expect(page.getByText(`완료 ${mockInitOrders.filter((d) => d.is_done).length}`)).toBeVisible();

    // 9. 주문 탭에서 '완료' 헤더 탭 클릭
    await page.getByText(`완료 ${mockInitOrders.filter((d) => d.is_done).length}`).click();

    // 10. 완료된 주문 카드 정보 확인
    const afterRequestOrders = mockInitOrders.filter((d) => d.is_done);
