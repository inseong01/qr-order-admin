...
import { FIRST_TABLE_POS, MODAL_QR_CODE_POS, SECOND_TABLE_POS, TEST_PAGE_URL } from './const';
...
test.describe('[성공] 좌석 관리', () => {
   ...
   */
  tableTabAPITest_S1('신규 좌석 생성 성공 - 생성 반영되고 새로고침 후에도 반영', async ({ page }) => {
    // 1. 좌석 탭 확인
   ...
   * - 사용자 조작, UI 반영, 스냅샷 확인
   */
  tableTabAPITest_S2('좌석 수정 성공 - 수정 반영되고 새로고침 후에도 반영', async ({ page }) => {
    // 1. 좌석 탭 진입 스냅샷
    await expect(page).toHaveScreenshot('init-tab.png');
   ...
   */
  tableTabAPITest_S3('좌석 삭제 성공 - 삭제 반영되고 새로고침 후에도 반영', async ({ page }) => {
    // 1. 좌석 탭 진입 스냅샷
    await expect(page).toHaveScreenshot('init-tab.png');
   ...
   */
  tableTabAPITest_S4('좌석 클릭 - 모달 및 QR 코드 표시되고 다운로드 정상 동작', async ({ page }) => {
    // 1. 좌석 탭 확인 및 클릭
    await expect(page).toHaveScreenshot('init-tab.png');
    await page.mouse.click(FIRST_TABLE_POS.X, FIRST_TABLE_POS.Y);
