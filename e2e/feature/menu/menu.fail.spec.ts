...
import { generateFakeImage, mockMenu, newCategory, newMenu, updatedCategory } from './const';
...
test.describe('[실패] 메뉴 관리', () => {
   ...
   */
  menuTabAPITest_S6('메뉴/카테고리 생성 실패 - API 오류 및 토스트 알림', async ({ page }) => {
    // 1. 위젯 열기 → '분류 추가' 클릭
   ...
   */
  menuTabAPITest_S7('메뉴 수정 실패 - API 오류 및 토스트 알림', async ({ page }) => {
    // 1. 수정할 메뉴 클릭
   ...
   * - 입력, PATCH 오류 모킹, 토스트 알림 확인
   */
  menuTabAPITest_S8('카테고리 수정 실패 - API 오류 및 토스트 알림', async ({ page }) => {
    // 1. 위젯 열기 → 분류 수정 클릭
    await page.getByText('열기').click();
   ...
   */
  menuTabAPITest_S9('메뉴 삭제 실패 - API 오류 및 토스트 알림', async ({ page }) => {
    // 1. 삭제할 메뉴 클릭
    await page.getByText(mockMenu.name).click();
   ...
   * - 사용자 조작, DELETE 오류 모킹, 토스트 알림 확인
   */
  menuTabAPITest_S10('카테고리 삭제 실패 - API 오류 및 토스트 알림', async ({ page }) => {
    // 1. 위젯 열기 → 분류 삭제 클릭
    await page.getByText('열기').click();
   ...
   */
  menuTabAPITest_S11('메뉴 생성 실패 - 이미지 업로드 오류 및 토스트 알림', async ({ page }) => {
    // 1. 메뉴 추가 버튼 클릭
    await page.getByText('메뉴 추가').click();
   ...
   */
  menuTabAPITest_S12('메뉴 수정 실패 - 이미지 업로드 오류 및 토스트 알림', async ({ page }) => {
    // 1. 수정할 메뉴 클릭
   ...
   * - 사용자 조작, DELETE 오류 모킹, 토스트 알림 확인
   */
  menuTabAPITest_S13('메뉴 삭제 실패 - 이미지 삭제 오류 및 토스트 알림', async ({ page }) => {
    // 1. 삭제할 메뉴 클릭
    await page.getByText(newMenu.name).click();
