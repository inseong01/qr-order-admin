...
test.describe('[성공] 메뉴 관리', () => {
  ...
  menuTabAPITest_S3('메뉴 카테고리 수정 - 메뉴와 카테고리 수정 정상 반영', async ({ page }) => {
    ...
    // 5. 확인 모달 '예' 클릭 및 POST 요청 확인 (메뉴 목록은 cascade 처리)
    await expect(page.getByText('예')).toBeVisible();
    await Promise.all([
      await page.getByText('예').click(),
      await page.waitForRequest(async (req) => req.method() === 'POST' && req.url().includes('/rest/v1/menu_category')),
    ]);
